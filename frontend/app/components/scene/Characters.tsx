// TELL Courtroom — characters & props, ported 1:1 from
// docs/tell/design_handoff_courtroom/prototype/courtroom-characters.jsx
// (Character + Face, Gavel, Scales, MiniIcon, exprFromSus).

import type { SVGProps } from "react";
import type { IconKind } from "../../lib/identities";

export type Expr = "calm" | "watch" | "sus" | "alarm";
export type Role = "judge" | "prosecutor" | "witness" | "juror";

// expression params from suspicion level (witnesses panic a touch earlier)
export function exprFromSus(s: number, isWitness = false): Expr {
  if (isWitness) {
...
const EXPR: Record<Expr, ExprParams> = {
...
function Face({ expr, speaking }: { expr: Expr; speaking: boolean }) {
  ...
  if (speaking) {
    mouth = (
      <ellipse className="mouth-talk" cx="60" cy="84" rx="11" ry="8" fill="#0a0e16" stroke="#000" strokeWidth="2.5" />
    );
...
export function Character({
  ...
  const leanClass = expr === "alarm" ? "alarm" : expr === "sus" ? "lean-2" : expr === "watch" ? "lean-1" : "";
  return (
    <div className={"char " + leanClass}>
      <svg width={w * scale} height={h * scale} viewBox="0 0 132 150">
        {/* shoulders / torso */}
        <path
          d="M16 150 Q16 112 40 104 L92 104 Q116 112 116 150 Z"
          fill={role === "judge" ? "#11151f" : "#222a40"}
          stroke="#000"
          strokeWidth="4"
        ...
        {role === "prosecutor" && (
          <path d="M62 104 L66 132 L70 104 Z" fill={accent} stroke="#000" strokeWidth="2.5" />
        )}
        {/* neck */}
        ...
        <Face expr={expr} speaking={speaking} />
        {/* sweat */}
        {sweat && (
          <path
            className="sweat"
            d="M100 40 C96 47 95 49 95 51 a4 4 0 0 0 8 0 C103 49 102 47 100 40 Z"
            fill="#00CFFF"
            stroke="#000"
            strokeWidth="1.6"
          />
...
// gavel (judge)
export function Gavel({ banged }: { banged: boolean }) {
  return (
...
export function Scales({ p }: { p: number }) {
  ...
  return (
    <svg viewBox="0 0 320 200">
      ...
      {/* rigid beam + hanging pans, rotated as one (SVG transform attribute, not CSS) */}
      <g className="beam" transform={`rotate(${deg} 160 58)`}>
        ...
        <text x="48" y="115" style={{ fontFamily: "var(--font-display)" }} fontSize="13" fill="#000" textAnchor="middle">
          TRUTH
        </text>
        {/* LIES pan (right) */}
        ...
        <text x="272" y="115" style={{ fontFamily: "var(--font-display)" }} fontSize="12" fill="#fff" textAnchor="middle">
          LIES
        </text>
      </g>
    </svg>
...
export function MiniIcon({ kind }: { kind: IconKind }) {
  // only the shared presentation attributes — typed narrowly so the spread is
  // assignable to every SVG element below (circle/line/rect/polygon/polyline).
  const c: Pick<SVGProps<SVGElement>, "fill" | "stroke" | "strokeWidth" | "strokeLinecap" | "strokeLinejoin"> = {
    fill: "none",
    stroke: "#000",
    strokeWidth: 2.4,
    strokeLinecap: "round",
  ...
  if (kind === "glass")
    return (
      <svg viewBox="0 0 24 24" width="14" height="14">
        <circle cx="10" cy="10" r="6" {...c} />
        <line x1="14.5" y1="14.5" x2="20" y2="20" {...c} />
      </svg>
    );
  if (kind === "layers")
    return (
      <svg viewBox="0 0 24 24" width="14" height="14">
        <polygon points="12,3 21,8 12,13 3,8" {...c} />
        <polyline points="3,12 12,17 21,12" {...c} />
      </svg>
    );
  if (kind === "doc")
    return (
      <svg viewBox="0 0 24 24" width="14" height="14">
        <rect x="5" y="3" width="14" height="18" {...c} />
        <line x1="8" y1="9" x2="16" y2="9" {...c} />
        <line x1="8" y1="13" x2="16" y2="13" {...c} />
      </svg>
    );
  return (
    <svg viewBox="0 0 24 24" width="14" height="14">
      <polyline points="2,12 7,12 10,4 14,20 17,12 22,12" {...c} />
    </svg>
  );
}
