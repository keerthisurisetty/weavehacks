...
export const DEMO_ROUNDS: DemoRound[] = [
  ...
  {
    id: "r_outage",
    topic: "The Friday-night outage on the payments API",
    mode: "lying",
    secret: "The engineer pushed an untested hotfix at 9:52pm that took down the payments API, then claimed they made no deploys during the change-freeze window.",
    blurb: "An on-call engineer covering their tracks. Holds composure for four questions — then the deploy log lands.",
    script: [
      { k: "phase", phase: "SETUP", banner: "WITNESS BRIEFING…" },
      { k: "beat", ms: 500 },
      { k: "phase", phase: "INTERROGATING" },
      { k: "say", who: "speaker", text: "Of course. I was on call Friday night. The payments API went down around ten, and I led the response from the first page through to recovery." },
      { k: "sig", d: "ce", s: 0.11, status: "uncertain", r: "Calm, takes ownership of the response. Nothing alarming — establishing the timeline before I press on it." },
      { k: "sig", d: "ca", s: 0.08, status: "confident", r: "Baseline logged: outage ~22:00, witness was the on-call responder. Consistent so far." },
      { k: "say", who: "ce", text: "Walk me through the hour before the first page. What were you working on?" },
      { k: "say", who: "speaker", text: "Mostly monitoring dashboards and clearing a few low-priority tickets. Quiet evening, right up until the alerts started firing." },
      { k: "sig", d: "ba", s: 0.16, status: "confident", r: "Relaxed phrasing, 'quiet evening.' No hedging yet. Reads as someone recounting an ordinary shift." },
      { k: "sig", d: "ce", s: 0.18, status: "uncertain", r: "'Mostly' is doing some work in that sentence. Noting it — not committing to anything yet." },
      { k: "say", who: "ce", text: "Did you deploy anything to production that evening?" },
      { k: "say", who: "speaker", text: "No. We were inside the Friday change-freeze window — no deploys go out during a freeze. I'd never push during one." },
      { k: "sig", d: "ca", s: 0.24, status: "uncertain", r: "Strong, absolute denial: 'no,' 'never.' Logging it as a hard claim I can cross-check against the deploy log." },
      { k: "sig", d: "ba", s: 0.27, status: "uncertain", r: "Confidence ticks up exactly on the deploy question. The categorical 'never' is louder than the rest of the answer." },
      { k: "say", who: "ce", text: "Who owned the change-freeze that night, and when did it start?" },
      { k: "say", who: "speaker", text: "Release management owns it. It kicked in at 6pm Friday and runs through Monday morning — standard policy." },
      { k: "sig", d: "ev", s: 0.31, status: "uncertain", r: "Freeze policy checks out: 18:00 Fri–Mon, owned by release mgmt. The policy is real. Now pulling the actual deploy events." },
      { k: "sig", d: "ce", s: 0.34, status: "uncertain", r: "Answer is fluent and correct on policy — but policy isn't the question. Whether HE deployed is. Bringing in the log." },
      { k: "say", who: "ce", text: "The deploy log shows a hotfix tagged to your account at 9:52pm — eight minutes before the outage. What was that?" },
      { k: "say", who: "speaker", text: "That… must be a mistake in the tagging. I don't remember pushing anything. The log can attribute things oddly sometimes." },
      { k: "sig", d: "ca", s: 0.88, status: "flagging", r: "CONTRADICTION: 'no deploys, never during a freeze' vs. deploy log — commit a1f9c, authored by this account, 21:52, eight minutes before the failure." },
      { k: "sig", d: "ev", s: 0.85, status: "flagging", r: "Deploy log + git blame + CI run all corroborate: an untested hotfix shipped at 21:52 under this account. The 'tagging mistake' has no support." },
      { k: "sig", d: "ba", s: 0.81, status: "flagging", r: "Composure breaks precisely on the load-bearing fact. 'Must be a mistake,' 'don't remember' — confidence collapses the moment the log appears." },
      { k: "sig", d: "ce", s: 0.8, status: "flagging", r: "Four clean answers, then a flat denial that the record refutes. The hotfix is the outage, and the denial is the lie." },
      { k: "beat", ms: 400 },
      { k: "phase", phase: "DELIBERATING", banner: "PANEL DELIBERATING…" },
      { k: "beat", ms: 1100 },
      { k: "verdict", label: "deceptive", conf: 0.86, decisive: "ca", contributing: ["ca", "ev", "ba", "ce"], head: "DECEPTION" },
      { k: "beat", ms: 600 },
      { k: "reveal" },
    ],
  },
